...
def get_content(single_content, name, *args):
    ...
    #爬取图片
    image_pattern_compile = re.compile(
        r'videoImgUrl="(.*?\.jpg).*?"')
    image_search = re.search(image_pattern_compile,sources)
    if image_search:
        image_down_url = image_search.group(1)
        #print('image_down_url')
        #print(image_down_url)
        image = {'name':'','url':image_down_url}
        images.append(image)
    # 如果是视频的话
    if single_content[1] == '1':
        try:
            ...
            video_down_url = re.search(download_pattern_compile,
                                       sources).group(1)
        video = {'name':name + '.mp4','url':video_down_url}
        videos.append(video)
    ...
    # 如果是文档的话
        ...
        doc = {'name':name + '.pdf','url':pdf_download_url}
        docs.append(doc)
...
def main():
    ...
    while True:
        ...
        course.set_course(cid)
        course.get_course_info()
        if check_select_course(course) == 'y':
            break
    get_course_all_source(course.course_id)
    resources = {'videos':videos,'docs':docs,'images':images}
    resourcesJson = json.dumps(resources,ensure_ascii=False)
    print(resourcesJson)
    #print('end--------')
